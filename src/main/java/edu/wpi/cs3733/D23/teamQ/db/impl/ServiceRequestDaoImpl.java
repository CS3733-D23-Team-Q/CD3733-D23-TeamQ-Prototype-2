    ...
    public List<ServiceRequest> getAllRows() {
        try {
            ...
            for (int i = 0; i < flowerRequests.size(); i++){
                ...
                ServiceRequest s = new ServiceRequest(fr.getRequestID(), fr.getRequester(), fr.getProgress(), fr.getAssignee(), fr.getRoomNumber(), fr.getSpecialInstructions());
                srL.add(s);
            }
            for (int i = 0; i < conferenceRequests.size(); i++){
                ConferenceRequest cr = conferenceRequests.get(i);
                ServiceRequest s = new ServiceRequest(cr.getRequestID(), cr.getRequester(), cr.getProgress(), cr.getAssignee(), cr.getRoomNumber(), cr.getSpecialInstructions());
                srL.add(s);
            }
            return srL;
